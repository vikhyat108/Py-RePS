...
with open('parameters_1F.txt', 'r') as file:
    for line in file:
        line = line.strip() 
        ...
        try:
            ...
            lcdm_settings[key] = float(value) if value.replace('.', '', 1).isdigit() else value
        except ValueError:
...
tk_minus = cosmo.get_transfer(98.9)
dbminus = tk_minus['d_b']
dcminus = tk_minus['d_cdm']
if N_nu == 0.0:
	dnminus =  [0]*knum
else:
	dnminus = tk_minus['d_ncdm[0]']

tk_plus = cosmo.get_transfer(99.1)
dbplus = tk_plus['d_b']
dcplus = tk_plus['d_cdm']
if N_nu == 0.0:
	dnplus =  [0]*knum
else:
	dnplus = tk_plus['d_ncdm[0]']
...
tk_minus = cosmo.get_transfer(99.0)
k1 = tk_minus['k (h/Mpc)']
...
for j in range(1,50):
	tk_minus = cosmo.get_transfer(zminus[j-1])
	k1 = tk_minus['k (h/Mpc)']
	dbminus = tk_minus['d_b']
	dcminus = tk_minus['d_cdm']
	if N_nu == 0.0:
	...
	tk_plus = cosmo.get_transfer(zplus[j-1])
	dbplus = tk_plus['d_b']
	dcplus = tk_plus['d_cdm']
	if N_nu == 0.0:
		dnplus =  [0]*knum
...
FB = np.interp(k,k1,y_b)
FC = np.interp(k,k1,y_c)
FN = np.interp(k,k1,y_n)
knum  = len(k)
...
for i in range(0,knum):
	...
	delta_m_99 = Ob0/(Ob0+Oc0)*delta_b_99 + Oc0/(Ob0+Oc0)*delta_c_99
	
	growth_b_99[i] = -xb_99[i]/db_99[i]
	growth_c_99[i] = -xc_99[i]/dc_99[i]
	#growth_n_99[i] = -xn_99[i]/dn_99[i]
	growth_m_99 = -(fc*xc_99[i] + fb*xb_99[i])/(fc*dc_99[i] + fb*db_99[i])
	
	Pm_99 = (delta_m_99)*(delta_m_99)*pm_0[i]
	
	tk_b = delta_b_99/(fb*delta_b_99 + fc*delta_c_99)
	tk_c = delta_c_99/(fb*delta_b_99 + fc*delta_c_99)
